use {
    crate::{
        ...
        util::format::{self, series, CodeStr},
    },
...
// Parse config data.
pub fn parse(task_file_data: &str) -> SealedResult<TaskFile> {
    // Deserialize the data.
    let task_file: TaskFile = serde_yaml::from_str(task_file_data)
        .map_err(|e| SealedError::System(format!("{e}"), None))?;
...
fn check_dependencies<'a>(task_file: &'a TaskFile) -> SealedResult<()> {
...
#[allow(clippy::too_many_lines)]
fn check_task(name: &str, task: &Task) -> SealedResult<()> {
...
mod tests {
    use {
        super::{
            ...
            MappingPath, Task, TaskFile, DEFAULT_LOCATION, DEFAULT_USER,
        },
