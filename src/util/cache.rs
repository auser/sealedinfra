use {
    crate::{
        ...
        task::taskfile::{command, location, user, Task, TaskFile},
    },
    sha2::{Digest, Sha256},
    std::{
        collections::HashMap,
...
#[cfg(test)]
mod tests {
    use super::{combine, hash_read, image_name, CryptoHash};
    use {
        crate::task::taskfile::{Task, TaskFile, DEFAULT_LOCATION, DEFAULT_USER},
        std::{collections::HashMap, path::Path},
        typed_path::UnixPath,
    };

    fn toastfile_with_task(foo_task: Task) -> TaskFile {
    ...
    fn toastfile_with_tasks(foo_task: Task, bar_task: Task) -> TaskFile {
        let mut tasks = HashMap::new();
        tasks.insert("foo".to_owned(), foo_task);
