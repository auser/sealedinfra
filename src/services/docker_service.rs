...
use crate::{
    ...
    services::exec_service::{run_attach, run_loud, run_quiet, run_quiet_stdin},
    task::taskfile::MappingPath,
    util::fs_utils::make_dirs,
};

const ERROR: Emoji<'_, '_> = Emoji("❌", "");
...
pub fn image_exists(
    ...
    match run_quiet(
        ...
        Ok(_) => Ok(true),
        Err(SealedError::Interrupted) => Err(SealedError::Interrupted),
...
fn rename_or_copy_file_or_symlink(
    ...
    if rename(source_path, destination_path).is_err() {
        ...
        if metadata.file_type().is_symlink() {
            ...
            #[cfg(windows)]
            return Err(failure::SealedError::System(
                format!(
