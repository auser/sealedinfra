...
fn add_file<R: Read, W: Write>(
    ...
    builder
        ...
        .map_err(|error| {
            SealedError::System(
                "Error appending data to tar archive.".to_owned(),
                Some(Box::new(error)),
            )
        })?;
...
fn add_symlink<W: Write>(
    ...
    header
        ...
        .map_err(|error| {
            SealedError::System(
                "Error appending symbolic link to tar archive.".to_owned(),
                Some(Box::new(error)),
            )
        })?;
...
fn add_directory<W: Write>(builder: &mut Builder<W>, path_rcr: &UnixPath) -> SealedResult<()> {
...
fn add_path<W: Write>(
    ...
    if metadata.file_type().is_file() {
        ...
        // archive.
        let mut file = File::open(path_cd).map_err(|error| {
            SealedError::System(
                format!(
                    "Unable to open file {}.",
        ...
        let target_path_std = read_link(path_cd).map_err(|error| {
            SealedError::System(
                format!(
...
pub fn create<W: Write>(
    ...
    for input_path_rsd in input_paths_rsd {
        ...
        let input_path_metadata = symlink_metadata(&input_path_cd).map_err(|error| {
            SealedError::System(
                format!(
        ...
        if input_path_metadata.is_dir() {
            ...
            loop {
                ...
                let entry_path_rsd = entry.path().strip_prefix(source_dir_cd).map_err(|error| {
                    SealedError::System(
                        format!(
    ...
    let builder = builder.into_inner().map_err(|error| {
        SealedError::System(
            "Error writing tar archive.".to_owned(),
            Some(Box::new(error)),
        )
    })?;
